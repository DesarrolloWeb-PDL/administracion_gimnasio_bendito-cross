    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <input
          ...
          className="flex-1 text-xl font-bold bg-transparent border-b-2 border-transparent focus:border-[var(--primary-color)] text-gray-800 dark:text-white outline-none py-1"
          placeholder="Título de la rutina semanal"
        />
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex-shrink-0 flex items-center gap-2 rounded-lg bg-[var(--primary-color)] px-5 py-2.5 text-sm font-medium text-white transition-colors hover:brightness-110 disabled:opacity-50"
        >
          {saving ? (
            <>
      ...
      <div className="overflow-x-auto pb-4">
        <div className="flex gap-4 min-w-max">
          {DAYS.map((d) => (
            <div key={d.key} className="w-[280px] flex-shrink-0">
              <DayColumn
                dia={d.key}
                diaLabel={d.label}
