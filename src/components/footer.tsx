export default function Footer() {
  return (
    <footer className="fixed bottom-0 left-0 w-full z-50 backdrop-blur-md bg-black/60 border-t border-zinc-800/50">
      <div className="flex items-center justify-center gap-2 py-3 px-4">
        <p className="text-xs text-zinc-400">&copy; 2026 Bendito Cross. Todos los derechos reservados.</p>
        <span className="text-zinc-600">|</span>
        <a
          href="https://desarrolloweb-pdl.vercel.app"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-orange-500 transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="currentColor" className="w-3.5 h-3.5">
            <rect x="4" y="4" width="11" height="11" rx="2" opacity="1" />
            <rect x="17" y="4" width="11" height="11" rx="2" opacity="0.7" />
            <rect x="4" y="17" width="11" height="11" rx="2" opacity="0.5" />
            <rect x="17" y="17" width="11" height="11" rx="2" opacity="0.3" />
          </svg>
          DesarrolloWeb-pdl
        </a>
      </div>
    </footer>
  );
}
